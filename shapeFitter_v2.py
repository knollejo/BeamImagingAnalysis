    ...
    with RootFile(inputfile) as f:
        for par in model.parameters():
            ...
            if supermod and match('^rho[MW][12]$', parname):
                ...
                not supermod and match('^rho[NMW][12]$', parname)
...
def main():
    ...
    bcid = int(argv[2])
    if len(argv) < 4 or not argv[3] or argv[3] not in (
        'SupG', 'SupDG', 'SG', 'DG', 'TG'
    ):
    ...
    if 'heavyion' in config and config['heavyion']:
        ...
        crange = (-10.0, 10.0)
    model = {
        'SupG': SuperGaussFit, 'SupDG': SuperDoubleGaussFit, 'SG': SingleGauss,
        'DG': DoubleGaussFit, 'TG': TripleGaussFit
    }[argv[3]](crange=crange)
    if len(argv)>4 and argv[4] and argv[4][0] == 'v' and argv[4][1:].isdigit():
        version = int(argv[4][1:])
    else:
        version = 1
    name = config['name']
    datafile = '{0}/{1}.root'.format(config['datapath'], config['dataname'])
    inputfile = 'BeamImaging_v{0}_{1}_{3}_bcid{2}'.format(
        version, name, bcid, {
            'SupG': 'SG', 'SupDG': 'DG', 'SG': 'noCorr', 'DG': 'SG', 'TG': 'DG'
        }[argv[3]]
    )
    nbins = config['nbins']
